      ...
      body: ListView(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _PowerButton(action: 'start', icon: Icons.play_arrow, activeAction: activeAction, onPressed: _power),
              _PowerButton(action: 'restart', icon: Icons.refresh, activeAction: activeAction, onPressed: _power),
              _PowerButton(action: 'stop', icon: Icons.stop, activeAction: activeAction, onPressed: _power),
              _PowerButton(action: 'kill', icon: Icons.power_settings_new, activeAction: activeAction, onPressed: _power),
            ],
          ),
          if (message != null) ...[
...
class _PowerButton extends StatelessWidget {
  ...
  const _PowerButton({required this.action, required this.icon, required this.activeAction, required this.onPressed});

  @override
  Widget build(BuildContext context) {
    final loading = activeAction == action;
    return OutlinedButton.icon(
      onPressed: activeAction == null ? () => onPressed(action) : null,
      icon: loading
